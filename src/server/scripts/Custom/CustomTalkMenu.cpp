...
void CustomTalkMenu::OnSelectMenuItem(Player* player, uint32 index)
{
	...
	if (index >= pMenu->items.size())
	{
		player->PlayerTalkClass->SendCloseGossip();
		pMenu->items.clear();
		player->Whisper(std::string("Select menu error!"), Language::LANG_COMMON, player->GetGUID());
		return;
	}
	...
	else
	{
		...
		default:
			...
			player->PlayerTalkClass->SendCloseGossip();
			pMenu->items.clear();
			player->Whisper(std::string("Unknow menu command!"), Language::LANG_COMMON, player->GetGUID());
			return;
		}
		if (!success)
		{
			player->Whisper(std::string("Execute menu error!"), Language::LANG_COMMON, player->GetGUID());
		}
		else
		{
			//if (pSelectItem->price > 0)
...
bool CustomTalkMenu::ProcessTeleport(CustomItem* pMenuItem, Player* player)
{
	...
	{
		player->Whisper(std::string("Combating not used!"), Language::LANG_COMMON, player->GetGUID());
		return true;
	}
