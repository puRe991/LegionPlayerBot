...
uint8 PlayerAI::GetPlayerSpec(Player* who)
{
    if (!who)
        return 0;
...
bool PlayerAI::IsPlayerHealer(Player* who)
{
    if (!who)
        return false;
...
bool PlayerAI::IsPlayerRangedAttacker(Player* who)
{
    if (!who)
        return false;
    ...
    switch (who->getClass())
    {
        ...
        case CLASS_HUNTER:
        {
            ...
            if (ItemTemplate const* rangedTemplate = rangedSlot ? rangedSlot->GetTemplate() : nullptr)
                if ((1 << rangedTemplate->BasicData->SubclassID) & ITEM_SUBCLASS_MASK_WEAPON_RANGED)
                    return true;
            return false;
        }
        case CLASS_PRIEST:
...
inline void PlayerAI::DoCastAtTarget(TargetedSpell spell)
{
...
void PlayerAI::DoRangedAttackIfReady()
{
    ...
    {
        switch (rangedTemplate->BasicData->SubclassID)
        {
...
{
    auto shapeshiftAuras = me->GetAuraEffectsByType(SPELL_AURA_MOD_SHAPESHIFT);
    std::set<Aura*> removableShapeshifts;
    for (auto itr = shapeshiftAuras->begin(); itr != shapeshiftAuras->end(); ++itr)
    {
...
struct UncontrolledTargetSelectPredicate : public std::unary_function<Unit*, bool>
{
    ...
    {
        return !target->HasCrowdControlAura();
    }
};
    ...
    switch (me->getClass())
    {
        ...
        case CLASS_ROGUE:
        {
            ...
            if (Unit* victim = me->getVictim())
            {
                ...
                uint8 const cp = /*(me->GetCom() == victim->GetGUID()) ?*/ me->GetComboPoints();
                if (cp >= 4)
...
void SimpleCharmedPlayerAI::UpdateAI(const uint32 diff)
{
    ...
    if (charmer->isInCombat())
    {
        ...
        if (!target || !charmer->IsValidAttackTarget(target) || target->HasCrowdControlAura())
        {
